...
public class Lags {
    ...
    public int revenue() {
        Comparator<Order> cm = (a,b) -> a.getStart() - b.getStart();
    ...
    private List<Order> selection(List<Order> orders, Order order) {
        List<Order>select = new ArrayList<>(orders);
        ...
        if(end > year_end) {
            ...
            final_end = (year + 1) * 1000 + days;
        }
        else {
            final_end = end;
        }
        Predicate<Order> pr = o-> (o.getStart() < final_end);
        select.removeIf(pr);
        return select;
    }
}
